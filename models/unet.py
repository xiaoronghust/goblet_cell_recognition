...
def unet_backbonenet(input_shape, backbone, use_multitask = False, stage_num = None):
    ...
    if use_multitask:
        pred1 = Deconv2D(filters = 1, kernel_size = [2,2], strides = [2,2], padding = 'same', activation = 'sigmoid', kernel_regularizer = l2(0))(conv_tensor)
        pred2 = Deconv2D(filters = 1, kernel_size = [2,2], strides = [2,2], padding = 'same', activation = 'sigmoid', kernel_regularizer = l2(0))(conv_tensor)
...
def TernausNet(input_shape, backbone, use_multitask = False, stage_num = None):
    ...
    conv_tensor = bn_relu(conv_tensor)
    if use_multitask:
        pred1 = Deconv2D(filters = 1, kernel_size = [2,2], strides = [2,2], padding = 'same', activation = 'sigmoid')(conv_tensor)
        pred2 = Deconv2D(filters = 1, kernel_size = [2,2], strides = [2,2], padding = 'same', activation = 'sigmoid')(conv_tensor)
        return Model(backbone_net.input, [pred1, pred2])
    else:
        pred = Deconv2D(filters = 1, kernel_size = [2,2], strides = [2,2], padding = 'same', activation = 'sigmoid')(conv_tensor)
        return Model(backbone_net.input, pred)
