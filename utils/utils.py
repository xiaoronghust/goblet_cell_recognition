...
def get_random_data(annotation_line, input_shape, max_boxes=50, jitter=.1, hue=.1, sat=1.5, val=1.5):
    '''random preprocessing for real-time data augmentation'''
    ...
    # object_num = box.shape[0]

    # 对图像进行缩放并且进行长和宽的扭曲
    new_ar = w/h * rand(1-jitter,1+jitter)/rand(1-jitter,1+jitter)
    scale = rand(0.9 ,1.1)*UPSAMPLE_RATIO # default .5 1.5
    if new_ar < 1:
        nh = int(scale*h)
        nw = int(nh*new_ar)
    else:
    ...
    # 翻转图像
    flip1 = rand()<.5
    if flip1: image = image.transpose(Image.FLIP_LEFT_RIGHT)
    flip2 = rand()<.5
    if flip2: image = image.transpose(Image.FLIP_TOP_BOTTOM)
    ...
    # image = image + np.random.random(3)*10 - 20
    # image_data = np.clip(image, 0, 255)/255.
    ...
    if len(box)>0:
        ...
        box[:, [0,2]] = box[:, [0,2]]*nw/iw + dx
        box[:, [1,3]] = box[:, [1,3]]*nh/ih + dy
        if flip1: box[:, [0,2]] = w - box[:, [2,0]]
        if flip2: box[:, [1,3]] = h - box[:, [3,1]]
        box[:, 0:2][box[:, 0:2]<0] = 0
        box[:, 2][box[:, 2]>w] = w
        box[:, 3][box[:, 3]>h] = h
        box_w = box[:, 2] - box[:, 0]
